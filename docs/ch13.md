...
<Figures figure="13-1">Modules resolved at run-time when running easytext.gui. Grayed-out modules are available in the JDK but not resolved.</Figures>
...
<Figures figure="13-2">A custom runtime image contains only the modules necessary for the application</Figures>
